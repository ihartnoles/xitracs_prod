...
class NewhiresController < ApplicationController
  ...
  def assign_to
     ...
     #exception to handle pscarlat
     if (params[:assigned_to] == "25") #pscarlat       
        newhirecourse.update_attributes(:assigned_to => 134)#pscarlat_r
     else
     ...
     if current_user.permission_id == 16
      ...
      if current_user.name == "kwrigh59"
        ...
        if current_user.permission_id == 8
          ...
          @send_to_notify=User.find_by_sql(['select id, concat(name,"@fau.edu") as displayname from users where id <> 134 order by name'])     
          @send_to_correct=User.find_by_sql(['select id, concat(name,"@fau.edu") as displayname from users where id <> 134 order by name'])     
        end
  ...
  def send_review_msg
     ...
     @message.created_at = Time.now.to_s(:db)
     @message.save
     ...
     @body = "A review has been entered for #{@newhirename} - #{@semester}"
  ...
  def save_signoff
          if !params[:newhiresignoff][:signed_off] && !params[:final_approval]
            ...
            #exception for pscarlat
            if current_user.name == 'mwalsh8' && signoff.sentto_id.to_i == 25
               signoff.sentto_id =  134            
            end

            @newhirecourse.save
            
            signoff.created_at = Time.now.to_s(:db)
            signoff.save
  ...
  def save_review
    ...
    if (params[:newhirereviewreason][:review_state] == 2 && params[:newhirereviewreason][:review_comments].blank?) 
        ...
        @newhirereason.created_at = Time.now_to_s(:db)
